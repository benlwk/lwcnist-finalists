...
//wklee, this is slower
__device__ void sparkle_opt_sm(uint32_t *state, int nb, int ns)
{
  int i, j;  // Step and branch counter
  uint32_t rc, tmpx, tmpy, x0, y0, tid = threadIdx.x;
  __shared__ uint32_t s_RCON[MAX_BRANCHES];
...
__device__ void sparkle_opt(uint32_t *state, int nb, int ns)
{
  int i, j;  // Step and branch counter
  uint32_t rc, tmpx, tmpy, x0, y0;
  
// #pragma unroll //wklee, need to fix ns to SPARKLE_STEPS_SLIM or BIG
  for(i = 0; i < ns; i ++) {
    // Add step counter
...
static __device__ void add_msg_blk(uint32_t *state, const uint8_t *in, int aligned)
{
...
static __device__ void add_msg_blk_last(uint32_t *state, const uint8_t *in, size_t inlen)
{
  uint32_t buffer[RATE_WORDS];
  uint8_t *bufptr;
  uint32_t tmpx = 0, tmpy = 0;
...
__device__ void Initialize_opt(uint32_t *state)
{
  int i;
...
__device__ void ProcessMessage_opt(uint32_t *state, const UChar *in, size_t inlen)
{
  ...
  while (inlen > RATE_BYTES) {
    ...
    // sparkle_opt_sm(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
    inlen -= RATE_BYTES;
    in += RATE_BYTES;
  }
  ...
  // sparkle_opt_sm(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void Finalize_opt(uint32_t *state, UChar *out)
{
  size_t outlen;
