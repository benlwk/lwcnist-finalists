...
__global__ void crypto_hash_gpu_ref(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  Initialize(&state);
  ProcessMessage(&state, in + bid*BLOCK_SIZE*MLEN + tid*MLEN, insize);
  Finalize(&state, out+ bid*BLOCK_SIZE*CRYPTO_BYTES + tid*CRYPTO_BYTES);

}

__global__ void crypto_hash_gpu_opt(UChar *out, const UChar *in, ULLInt inlen)
{
  uint32_t tid = threadIdx.x, bid = blockIdx.x;
  uint32_t state[STATE_WORDS];
  size_t insize = (size_t) inlen;

  Initialize_opt(state);
  ProcessMessage_opt(state, in + bid*BLOCK_SIZE*MLEN + tid*MLEN, insize);
  Finalize_opt(state, out+ bid*BLOCK_SIZE*CRYPTO_BYTES + tid*CRYPTO_BYTES);
}
...
int main(int argc, char* argv[]) {   
  ...
  cudaMalloc((void**)&d_state, (uint64_t) BATCH * SPARKLE_STATE/8 * sizeof(uint8_t));
  ...
  cudaMallocHost((void**)&h_state, (uint64_t) BATCH * SPARKLE_STATE/8 * sizeof(uint8_t));
...
#ifdef OPT
  printf("\n Timing Opt Sparkle...MLEN: %u\n", MLEN);      
  crypto_hash_gpu_opt<<<blocks, threads>>>(d_digest, d_msg, mlen);
#else
  printf("\n Timing Ref Sparkle...MLEN: %u\n", MLEN);      
...
#ifdef DEBUG
  for(k=0; k<4; k++) {printf("\n batch %u\n", k);for (i = 0; i <CRYPTO_BYTES; i++) {printf("%02x", h_digest[ k*CRYPTO_BYTES + i]);}}
#endif    
